...
Il codice descritto - per il suo funzionamento - si basa esclusivamente sulla funzione `tone()` di Arduino, pertanto il risultato non sarà degno
...
* la scrittura di un'ulteriore libreria, del tipo `nome-melodia.h` con la melodia desiderata;
* infine, il codice stesso, che esegue la melodia una sola volta, inserendola nel `setup`.

Pertanto, per il suo corretto funzionamento, la cartella del codice dovrà essere strutturata come segue:
...
e, naturalmente, occore ricordarsi di controllare queste due linee all'inizio del codice:
...
Questo codice non deve essere modificato. Contiene tutte le frequenze delle note musicali e `REST`, pari a $0~\text{Hz}$, utilizzato per le pause.
